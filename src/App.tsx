  ...
  const handleCreateBreakdown = async (req: BreakdownRequest) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(errJson?.error || 'AI breakdown failed. Please check your API key.');
        setIsLoading(false);
        return;
      }
    } catch (err: any) {
    ...
    if (!data || !data.steps || !Array.isArray(data.steps) || data.steps.length === 0) {
      setError('AI service did not return a valid task breakdown. Please try again or check your API key.');
      setIsLoading(false);
      return;
    }
